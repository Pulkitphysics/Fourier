//For triangle wave and giving coefficients of Fourier series also
//for running first define function acc to yourself which can show triangular wave feature using 'deff' in console and call the function Fplot in function.
funcprot(0);
clf;clc
function [A0,An,Bn]=Fplot(l,n,f3,f4)   //as function is divided in two parts so f3 and f4, as function limit id - l to l so 'l' is half of that period and 'n' is number of terms we needed.
    ...
    xgrid(5)
    legend("sol(x)")
    xtitle("Triangular waves through fourier series")
